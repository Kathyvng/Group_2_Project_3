...
/* ANH_HEALTH_STATISTICS_PAGE JAVASCRIPT BELOW */
...
/* KATHY_STATE_AND_TOPIC_PAGE JAVASCRIPT BELOW */
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadJsonData() {
        fetch('/General_Analysis/Kathy_LineChart/CDI_Avg_Topic_Questions.json')
            .then(response => response.json())
            .then(data => {
                jsonData = data;
    ...
    function populateDropdowns() {
        ...
        const defaultState = uniqueStates[0]; // Use the first state as default if it exists
        const defaultTopic = uniqueTopics[0]; // Use the first topic as default if it exists
            ...
            lineChart = new Chart(ctx, {
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y: { title: { display: true, text: "Avg CDI Data Value" }, beginAtZero: true }
                    }
                }
            });
        }
        ...
        if (lineChart2) {
            ...
            lineChart2 = new Chart(ctx2, {
                ...
                options: {
                    ...
                    scales: {
                        x: { title: { display: true, text: "Year" } },
                        y: { title: { display: true, text: "Avg CDI Data Value" } }
                    }
                }
            });
        }
